...
static PyTypeObject *postfixop_type;
static PyObject *JiaJia_singleton, *NoNoNo_singleton;
static PyObject* ast2obj_postfixop(postfixop_ty);
static PyTypeObject *JiaJia_type;
static PyTypeObject *NoNoNo_type;
...
static int init_types(void)
{
    ...
    Name_type = make_type("Name", expr_type, Name_fields, 3);
    ...
    postfixop_type = make_type("postfixop", &AST_type, NULL, 0);
    if (!postfixop_type) return 0;
    if (!add_attributes(postfixop_type, NULL, 0)) return 0;
    JiaJia_type = make_type("JiaJia", postfixop_type, NULL, 0);
    if (!JiaJia_type) return 0;
    JiaJia_singleton = PyType_GenericNew(JiaJia_type, NULL, NULL);
    if (!JiaJia_singleton) return 0;
    NoNoNo_type = make_type("NoNoNo", postfixop_type, NULL, 0);
    if (!NoNoNo_type) return 0;
    NoNoNo_singleton = PyType_GenericNew(NoNoNo_type, NULL, NULL);
    if (!NoNoNo_singleton) return 0;
...
static int obj2ast_postfixop(PyObject* obj, postfixop_ty* out, PyArena* arena);
...
expr_ty
Name(identifier id, expr_context_ty ctx, postfixop_ty postf, int lineno, int
     col_offset, int end_lineno, int end_col_offset, PyArena *arena)
{
    expr_ty p;
    if (!id) {
    ...
    if (!postf) {
        PyErr_SetString(PyExc_ValueError,
                        "field postf is required for Name");
        return NULL;
    }
    p = (expr_ty)PyArena_Malloc(arena, sizeof(*p));
    if (!p)
...
ast2obj_expr(void* _o)
{
    ...
    case Name_kind:
        ...
        if (_PyObject_SetAttrId(result, &PyId_ctx, value) == -1)
            goto failed;
        Py_DECREF(value);
        value = ast2obj_postfixop(o->v.Name.postf);
        if (!value) goto failed;
        if (_PyObject_SetAttrId(result, &PyId_postf, value) == -1)
            goto failed;
        Py_DECREF(value);
        break;
    case List_kind:
...
PyObject* ast2obj_postfixop(postfixop_ty o)
{
    switch(o) {
        case JiaJia:
            Py_INCREF(JiaJia_singleton);
            return JiaJia_singleton;
        case NoNoNo:
            Py_INCREF(NoNoNo_singleton);
            return NoNoNo_singleton;
        default:
            /* should never happen, but just in case ... */
            PyErr_Format(PyExc_SystemError, "unknown postfixop found");
            return NULL;
    }
}
...
obj2ast_expr(PyObject* obj, expr_ty* out, PyArena* arena)
{
    ...
    if (isinstance) {
        ...
        if (_PyObject_LookupAttrId(obj, &PyId_postf, &tmp) < 0) {
            return 1;
        }
        if (tmp == NULL) {
            PyErr_SetString(PyExc_TypeError, "required field \"postf\" missing from Name");
            return 1;
        }
        else {
            int res;
            res = obj2ast_postfixop(tmp, &postf, arena);
            if (res != 0) goto failed;
            Py_CLEAR(tmp);
        }
        *out = Name(id, ctx, postf, lineno, col_offset, end_lineno,
                    end_col_offset, arena);
        if (*out == NULL) goto failed;
        return 0;
    }
...
int
obj2ast_postfixop(PyObject* obj, postfixop_ty* out, PyArena* arena)
{
    int isinstance;

    isinstance = PyObject_IsInstance(obj, (PyObject *)JiaJia_type);
    if (isinstance == -1) {
        return 1;
    }
    if (isinstance) {
    ...
    isinstance = PyObject_IsInstance(obj, (PyObject *)NoNoNo_type);
    if (isinstance == -1) {
        return 1;
    }
    if (isinstance) {
    ...
    PyErr_Format(PyExc_TypeError, "expected some sort of postfixop, but got %R", obj);
    return 1;
}
...
PyInit__ast(void)
{
    ...
    if (PyDict_SetItemString(d, "postfixop", (PyObject*)postfixop_type) < 0)
        return NULL;
    if (PyDict_SetItemString(d, "JiaJia", (PyObject*)JiaJia_type) < 0) return
        NULL;
    if (PyDict_SetItemString(d, "NoNoNo", (PyObject*)NoNoNo_type) < 0) return
        NULL;
